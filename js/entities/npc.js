...
export function createNpc(tx, ty, fatherId = null, motherId = null) {
    ...
    let npc = {
        ...
        race: fatherId ? state.npcs.find(n=>n.id===fatherId).race : ['Nhân loại', 'Tiên', 'Người thú', 'Người lùn'][Math.floor(Math.random()*4)],
        traits: [],
    ...
    if (!fatherId) {
        if (Math.random() < 0.3) {
            let possibleTraits = ['Khỏe mạnh', 'Yếu ớt', 'Siêng năng', 'Lười biếng', 'Thông minh', 'Ngốc nghếch'];
